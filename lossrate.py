...
def usage():
    print "usage: python",sys.argv[0],"[-h/--help] [-4/--IPv4] [-6/--IPv6] [-v/--verbose] [-i any/--interface=em1] [-b/bypassCDN] [-c/--crawl] [-t1/--thread=1] [-q/---quota] URL\n"
...
def rttmeasure(domain,version=4):
    ...
    if(version==4):
        ip = '0.0.0.0'
        cmd = 'hping3 -S -p 80 -c 5 --fast '+ domain
...
class mea_thread(threading.Thread):
    ...
    def run(self):
        ...
        if(self.bypassCDN):
            ...
            cmd += '-'+self.version+' -T 10 -t 1 -o '+filepath+'.txt -O /dev/null http://'
            if(self.version=='6'):
                cmd=cmd+'['+self.ip+']'
            else:
                cmd=cmd+self.ip
...
if __name__=='__main__':
    try:
        opts,args = getopt.gnu_getopt(sys.argv[1:],"46hvt:i:bcq:u:",["ipv4","ipv6","help", "verbose", "thread=", "interface=", "bypassCDN", "crawl", "quota=", "URL="])
    ...
    if bypassCDN:
        ...
        snippet = domain
    tcpdump = "tcpdump -s 120 -i {0} host {1} -w {2}".format(interface, snippet, packets)
    ...
    if(est_result):
        ...
        if tshark==0:
            ...
            if ipv==6:
                tshark = "tshark -q -r "+packets+' -z io,stat,100,"ipv6.src=='+realip+'","tcp.analysis.lost_segment" '
            else:
                tshark = "tshark -q -r "+packets+' -z io,stat,100,"ip.src == '+realip+'","tcp.analysis.lost_segment" '
            if(latency>0):
                if(latency<10):
                    tshark = tshark.replace('io,stat,100','io,stat,0.01')
                else:
            ...
            if(interval and m):
                ...
                if(curdata>maxdata):
                    maxdata = curdata
                if(totalpacket):
                    lossrate = 100.0*lossinterval/totaldata*1514
                totaldata = int(totaldata *1460 / 1514.0) # payload / packet length
            #return (totaldata, maxdata, maxbw, serverslow, lossrate)
            print 'tcpdump statistics: latency:',latency,'ms, download size',totaldata,'B, peak speed:',maxbw,'B/s, loss rate:',lossrate,'%'
            print '-'*70,'\n'
